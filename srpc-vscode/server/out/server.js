  ...
  "node_modules/vscode-languageserver/lib/node/files.js"(exports2) {
    ...
    (function(FileSystem2) {
      ...
      function isParent(parent, child) {
        if (isCaseSensitive()) {
          return path5.normalize(child).indexOf(path5.normalize(parent)) === 0;
        } else {
          return path5.normalize(child).toLowerCase().indexOf(path5.normalize(parent).toLowerCase()) === 0;
        }
      }
...
var GENERIC_TYPE_KEYWORDS = ["map", "list"];
...
// server/src/types.ts
function visitTypeNodes(type, visitor) {
  visitor(type);
  switch (type.kind) {
    case "array":
    case "list":
...
function typeToString(type) {
  switch (type.kind) {
    ...
    case "inline-struct": {
      const fields = type.fields.map((f) => `${f.name}${f.optional ? "?" : ""}: ${typeToString(f.type)}`).join(", ");
      return `{ ${fields} }`;
    }
  }
}
...
var Parser = class {
  ...
  skipUntilNextDeclaration() {
    while (!this.eof()) {
      if (this.peekKeyword("enum") || this.peekKeyword("struct") || this.peekKeyword("service") || this.peekKeyword("package")) {
        return;
      }
      this.index++;
    }
...
function collectReferenceDiagnostics(document, diagnostics, resolveType = () => true) {
  ...
  for (const decl of document.declarations) {
    const qualified = document.packageName ? `${document.packageName}.${decl.name}` : decl.name;
  ...
  const checkType = (type) => {
    visitTypeNodes(type, (node) => {
      ...
      const simple = node.name.includes(".") ? node.name.split(".").pop() : node.name;
      if (!resolveType(node.name) && !defined.has(node.name) && !isScalarType(simple)) {
        diagnostics.push({
...
function collectSymbols(document) {
  ...
  for (const decl of document.declarations) {
    ...
    if (decl.kind === "service") {
      ...
      for (const method of decl.methods) {
        const httpPrefix = method.httpMethod ? `@${method.httpMethod.toLowerCase()} ` : "";
        const params = method.params.map((p) => `${p.name}: ${typeToString(p.type)}`).join(", ");
        symbols.push({
          kind: "method",
          name: method.name,
          range: method.nameSpan.range,
...
function formatDocument(source) {
  ...
  for (const decl of document.declarations) {
    ...
    if (decl.kind === "service") {
      ...
      for (const method of decl.methods) {
        const decorator = method.httpMethod ? `@${method.httpMethod.toLowerCase()} ` : "";
...
var CONTRACT_DIRECTORY_NAMES = /* @__PURE__ */ new Set(["contract", "contracts"]);
function findContractRootForUri(uri) {
...
function getCompletionContext(document, position, ast) {
  ...
  if (/:\s*[\w."'|]*$/.test(beforeCursor) || /=>\s*[\w."'|]*$/.test(beforeCursor) || /map<\s*[\w."'|]*$/.test(beforeCursor) || /map<\s*[\w."'|]*,\s*[\w."'|]*$/.test(beforeCursor) || /list<\s*[\w."'|]*$/.test(beforeCursor) || /\|\s*[\w."'|]*$/.test(beforeCursor)) {
    return "type";
  }
...
function buildCompletions(document, position, index) {
  ...
  if (context === "keyword") {
    ...
    for (const symbol of index.getAllSymbols()) {
      ...
      items.push({
        ...
        detail: `${symbol.kind} (in ${path2.basename(uriToPath(symbol.uri))})`,
  ...
  if (context === "type") {
    ...
    for (const generic of GENERIC_TYPE_KEYWORDS) {
      ...
      const snippet = generic === "map" ? "map<${1:string}, ${2:string}>" : `${generic}<\${1:string}>`;
      items.push({
        label: generic,
        kind: import_node.CompletionItemKind.Keyword,
        insertText: snippet,
...
function sanitizePackageSegment(name) {
  const cleaned = name.replace(/[^A-Za-z0-9_.]/g, "_").replace(/^([0-9])/, "_$1").toLowerCase();
...
function suggestUniquePackageName(index, uri) {
  ...
  const folder = sanitizePackageSegment(path3.basename(path3.dirname(filePath)));
...
function getGlobalNameDiagnostics(index) {
  ...
  for (const [name, entries] of byName) {
    ...
    for (const entry of uniqueEntries) {
      const others = uniqueEntries.filter((other) => other !== entry).map((other) => `${other.kind} in ${path3.basename(uriToPath(other.uri))}`).join(", ");
...
var CONTRACT_EXTENSIONS = /* @__PURE__ */ new Set([".ctr", ".rpc"]);
var CONTRACT_DIRECTORY_NAMES2 = /* @__PURE__ */ new Set(["contract", "contracts"]);
var WorkspaceIndex = class {
  files = /* @__PURE__ */ new Map();
  symbolsByQualifiedName = /* @__PURE__ */ new Map();
  symbolsBySimpleName = /* @__PURE__ */ new Map();
  symbolsByUri = /* @__PURE__ */ new Map();
  getFile(uri) {
  ...
  getUsedTopLevelNames(excludeUri) {
    ...
    const excluded = excludeUri ? normalizeUri(excludeUri) : void 0;
  ...
  findReferences(uri, position) {
    ...
    const targetNames = /* @__PURE__ */ new Set([symbol.name]);
  ...
  evictAliasFiles(uri) {
    const path5 = canonicalPathForUri(uri);
    for (const existingUri of [...this.files.keys()]) {
      if (existingUri === uri) {
        continue;
      }
      if (canonicalPathForUri(existingUri) === path5) {
  ...
  indexSymbols(uri, document) {
    ...
    for (const decl of document.declarations) {
      ...
      const qualifiedName = document.packageName ? `${document.packageName}.${decl.name}` : decl.name;
      const info = {
        name: decl.name,
        qualifiedName,
        kind: decl.kind,
...
async function findContractDirectories(rootDir) {
  const found = [];
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (CONTRACT_DIRECTORY_NAMES2.has(entry.name.toLowerCase())) {
        found.push(fullPath);
        continue;
      }
      await walk(fullPath);
...
async function walkDirectory(dir, uris) {
  ...
  for (const entry of entries) {
    ...
    if (CONTRACT_EXTENSIONS.has(path4.extname(entry.name))) {
...
function readFileUri(uri) {
  return fs2.readFileSync(uriToPath(uri), "utf8");
}
...
${symbol.detail}`;
  ...
  if (symbol) {
    ...
    if (global) {
      const relPath = path4.basename(uriToPath(global.uri));
...
Defined in \`${relPath}\``;
  ...
  if (localDecl) {
    const body = localDecl.kind === "enum" ? localDecl.values.map((v) => v.name).join(", ") : localDecl.kind === "struct" ? localDecl.fields.map((f) => `${f.name}${f.optional ? "?" : ""}: ${typeToString(f.type)}`).join("\n") : localDecl.methods.map((m) => m.name).join(", ");
...
var ORPHAN_KEY = "__srpc_orphan__";
var WorkspaceManager = class {
  folders = [];
  indexes = /* @__PURE__ */ new Map();
  trackedUris = /* @__PURE__ */ new Set();
  setFolders(folders) {
  ...
  async indexAll(documents2) {
    ...
    for (const doc of documents2.all()) {
      allUris.add(normalizeUri(doc.uri));
    }
    const urisByKey = /* @__PURE__ */ new Map();
    for (const uri of allUris) {
      const key = this.indexKeyForUri(uri);
      const bucket = urisByKey.get(key) ?? /* @__PURE__ */ new Set();
      bucket.add(uri);
    ...
    for (const [key, uris] of urisByKey) {
      ...
      for (const uri of uris) {
        try {
          const source = getOpenDocument(documents2, uri)?.getText() ?? readFileUri(uri);
  ...
  reloadFromDisk(uri, documents2) {
    const normalized = normalizeUri(uri);
    try {
      const source = getOpenDocument(documents2, normalized)?.getText() ?? readFileUri(normalized);
  ...
  hasFile(uri) {
    return this.getIndexForUri(uri).getFile(uri) !== void 0;
  }
  buildDiagnostics() {
    const results = /* @__PURE__ */ new Map();
...
function getOpenDocument(documents2, uri) {
  const normalized = normalizeUri(uri);
  return documents2.get(normalized) ?? documents2.get(uri);
}

// server/src/server.ts
var autoPackageEnabled = true;
...
connection.onInitialized(async () => {
  ...
  if (hasWorkspaceFolderCapability) {
    connection.client.register(
      import_node2.DidChangeWorkspaceFoldersNotification.type,
      void 0
    );
    connection.workspace.onDidChangeWorkspaceFolders(async (event) => {
      ...
      workspaceFolders = workspaceFolders.filter((folder) => !removed.has(normalizeUri(folder.uri))).concat(
...
function publishDiagnostics() {
  for (const [uri, rawDiagnostics] of workspaceManager.buildDiagnostics()) {
    const diagnostics = rawDiagnostics.map((d) => ({
      severity: d.severity === "error" ? import_node2.DiagnosticSeverity.Error : import_node2.DiagnosticSeverity.Warning,
      range: d.range,
      message: d.message,
      source: "srpc"
    }));
...
function indexForUri(uri) {
  return workspaceManager.getIndexForUri(uri);
}
function getOpenDocument2(uri) {
  const normalized = normalizeUri(uri);
...
async function maybeAutoInsertPackage(document) {
...
connection.onCompletion((params) => {
  const document = getOpenDocument2(params.textDocument.uri);
  if (!document) {
    return [];
  }
...
connection.onHover((params) => {
  ...
  if (!hover) {
    const document = getOpenDocument2(uri);
    const word = document ? getWordAtPosition2(document, params.position) : null;
...
connection.onRenameRequest((params) => {
  ...
  for (const ref of references) {
    changes[ref.uri] ??= [];
    changes[ref.uri].push(import_node2.TextEdit.replace(ref.range, params.newName));
  }
  return { changes };
});
