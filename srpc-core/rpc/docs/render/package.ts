...
export function renderPackageDocs(
  ...
  return page(
    ...
    `<article class="${cls.panel}">
      ...
      <p class="${cls.lead}">All APIs and data shapes for the <code class="${cls.code}">${escapeHtml(pkg.package)}</code> area of your app.</p>
      <p class="${cls.meta}">Defined in contract file <code class="${cls.codeXs}">${escapeHtml(pkg.file)}</code></p>
      <div class="${cls.stats}">
